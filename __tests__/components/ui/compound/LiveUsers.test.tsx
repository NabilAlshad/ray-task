...
describe("LiveUsers", () => {
  ...
  it("switches the current user from the modal without reloading", async () => {
    ...
    await user.click(
      screen.getByRole("radio", { name: "Choose teal 500 avatar color" }),
    );
    ...
    expect(window.localStorage.getItem("task-board-current-user")).toContain(
      "Casey",
    );
    expect(
      screen.queryByRole("heading", { name: "Switch User" }),
  ...
  it("shows preset users from json in the switch modal and lets you pick one", async () => {
    ...
    expect(
      screen.getByText("Preset users are loaded from `data/users.json`."),
    ).toBeInTheDocument();
    ...
    await user.click(
      screen.getByRole("button", { name: "Select saved user Bob 22" }),
    );
  ...
  it("shows the user management section as read-only for members and viewers", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Delete saved user Alice 12" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("lets admins delete a saved user from the switch modal", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Switch User" }));
    await user.click(
      screen.getByRole("button", { name: "Delete saved user Bob 22" }),
    );
