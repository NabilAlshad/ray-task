import { isValidUserRole } from "./usersDirectoryStore.js";
...
export function registerSocketHandlers({
  ...
  io.on("connection", (socket) => {
    ...
    if (socket.recovered) {
      ...
      socket.emit("initTasks", tasksStore.getAll());
      socket.emit("activeUsers", getActiveUsers().filter((user) => user.id !== socket.id));
    }
    ...
    socket.on("createUser", (user) => {
      if (socket.data.user?.role !== "ADMIN") {
        emitUserDirectoryError(socket, "create", "Only admins can create saved users.");
        return;
      }
    ...
    socket.on("deleteUser", (userId) => {
      if (socket.data.user?.role !== "ADMIN") {
        emitUserDirectoryError(socket, "delete", "Only admins can delete saved users.");
        return;
      }
